<script lang="ts">
	...
	const subFor = $derived.by(() => {
		...
		d.ingredients.forEach((entry, i) => {
			...
			const hit = data.substitutions.find((s) => lower.includes(s.term.toLowerCase()));
...
<article class="shell sheet">
	...
	<section class="pairing">
		...
		<dl>
			<div><dt>The Pour</dt><dd>{data.pairing.pour}</dd></div>
			<div><dt>Also Right</dt><dd>{data.pairing.alt}</dd></div>
			{#if data.pairing.beer !== '—'}
				<div><dt>From the Taps</dt><dd>{data.pairing.beer}</dd></div>
			{/if}
