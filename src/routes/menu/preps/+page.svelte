<script lang="ts">
	...
	function addLine() {
		...
		form.lines = [
			...form.lines,
			{ id: mintId('l-'), item: '', unitCost: 0, unit: 'kg', usedQty: 0, yieldPct: 100 }
		];
	}
				...
				<table class="lines">
					...
					<tbody>
						{#each form.lines as l (l.id)}
							<tr>
								<td><input value={l.item} onchange={(e) => editLine(l.id, { item: e.currentTarget.value })} /></td>
								<td><input type="number" step="0.01" value={l.unitCost} onchange={(e) => editLine(l.id, { unitCost: num(e.currentTarget.value) })} /></td>
