...
/** At most this many following lines can belong to the dish above as its description. */
 ...
 * whole number: that rule is what keeps 'Pinot Noir 2019' a dish name rather
 ...
 * dish name, where a person can see it and decide for themselves.
 */
...
type Kind = 'price' | 'market' | 'size' | 'connector' | 'currency' | 'other';
 ...
 */
function splitPrice(text: string): { name: string; price: string } {
 ...
 */
function takeMarks(text: string): { text: string; tags: string[] } {
	const tags: string[] = [];
	...
	let out = text.replace(/[([]([^)\]]{1,40})[)\]]/g, (whole, inside: string) => {
		...
		for (const m of marks) add(m as string);
		return ' ';
	});
 ...
 * 'Chips 4' has exactly the same shape and is a side the kitchen sells. The
 * caller insists on a blank line above and a priced line below before treating
 * one of these as a heading; parseMenuText says why that is the safer way round.
 */
function isWeakHeading(text: string): boolean {
	const core = stripDecoration(text);
 ...
 * what the dish is called, and because leaving it on was worse than untidy. The
 * dash form fed straight into the name and description split, which reads ' - '
 * as punctuation the kitchen chose in order to separate the two, so '14 - Egg
 * fried rice 3.90' came back as a dish called '14' carrying 'Egg fried rice' for
...
function scan(raw: string): Scan {
	const collapsed = raw
		...
		// line as printed, so a row worth checking still shows the cook the
...
function emit(s: Scan, section: string, dishes: ParsedDish[]): ParsedDish {
 ...
 * on screen is one click to delete and a dropped dish is invisible.
 *
 ...
 * of them as a heading leaves 'To Begin' sitting in the dish column. So a
...
export function parseMenuText(text: string): ParseResult {
	...
	/** @returns true when the next line worth reading is a priced dish. */
	const pricedLineFollows = (from: number): boolean => {
	...
	for (let i = 0; i < scans.length; i++) {
		...
		if (s.name === '') {
			// The line was nothing but a price, or nothing but marks. There is no dish
		...
		// A description running on below its dish. A heading-shaped line is not
		// allowed to be one, because gluing 'Bread' onto the chips above it would
		// lose a side the kitchen sells. An indented line is allowed to be one
